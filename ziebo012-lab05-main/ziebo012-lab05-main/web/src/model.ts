...
let mixers: {id: number, mixer: THREE.AnimationMixer, start?: number, duration?: number}[] = [];
...
function addEntity(id: number, details: any) {
  return new Promise<THREE.Group>((resolve, reject) => {
    gltfLoader.load(details.mesh, (gltf) => {
      $('#entity-select').append($(`<option value="${id}">${details.name}</option>`));
      ...
      if (animation) {
        ...
        mixers.push({id: id, mixer: mixer, start: details.start, duration: details.duration});
      ...
      if (details.offset) group.userData.offset = new THREE.Vector3(...details.offset);
...
function loadModel(details: any) {
  return new Promise<THREE.Group>((resolve, reject) => {
    objLoader.load(details.mesh, (obj) => {
      ...
      let roofMaterial = new THREE.MeshStandardMaterial({color: details.color, map: roofTexture});
...
function updateAnimations(delta: number) {
  mixers.forEach((mixer) => {
    if (mixer.start && mixer.duration) {
      if (mixer.mixer.time < mixer.start || mixer.mixer.time > mixer.start + mixer.duration) {
...
export { entities, addEntity, removeEntity, updateAnimations, updateEntity, addMesh, loadModel, ground };
