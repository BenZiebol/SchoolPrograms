...
let beamSelector1 = new THREE.Mesh(beamGeometry, new THREE.MeshBasicMaterial({ color: 0xffff00, transparent: true, opacity: 0.5 }));
let beamSelector2 = new THREE.Mesh(beamGeometry, new THREE.MeshBasicMaterial({ color: 0x00ff00, transparent: true, opacity: 0.5 }));
let beamState = 0;
let selectedStrat = 0;
...
function initScheduler() {
  ...
  container.onmousedown = (click) => {
    if (beamState > 0) {
      ...
      let normalized_coords = new THREE.Vector2((click.clientX / container.clientWidth) * 2 - 1, -(click.clientY / container.clientHeight) * 2 + 1);
  ...
  scheduleSubmit.onclick = () => {
    ...
    let searchStrat = (scheduleStrategy[selectedStrat] as HTMLOptionElement).value;
    scheduleError.innerHTML = "";
    if (name == "") scheduleError.innerHTML += '<p style="color: red">[!] Error, missing name...</p>'
    if (beamState != 3) scheduleError.innerHTML += '<p style="color: red">[!] Error, missing pickup and drop off location ...</p>'
    if (name != "" && beamState == 3) {
      ...
      sendCommand("ScheduleTrip", { name: name, start: [beamSelector1.position.x*14.2, beamSelector1.position.z*14.2], end: [beamSelector2.position.x*14.2, 254.665, beamSelector2.position.z*14.2], search: searchStrat });
      beamState = 0;
      beamSelector1.visible = false;
      beamSelector2.visible = false;
      scheduleButton.hidden = false;
  ...
  window.addEventListener("keydown", (ev) => {
    switch (ev.key) {
      ...
      case "ArrowUp":
        if (beamState > 0) {
          ev.preventDefault();
          selectedStrat = ((((selectedStrat + 1) % scheduleStrategy.length) + scheduleStrategy.length) % scheduleStrategy.length);
          (scheduleStrategy[selectedStrat] as HTMLOptionElement).selected = true;
        }
        break;
      case "ArrowDown":
        if (beamState > 0) {
